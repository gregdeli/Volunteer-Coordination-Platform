...
function myMessage($a1) {
    ...
    // $sql = "SELECT full_name, phone, latitude, longitude, num_people, date_submitted, name
    // FROM request INNER JOIN user ON civ_id=user.id INNER JOIN item on item_id=item.id
    // WHERE role='CIVILIAN' AND date_undertaken=NULL AND date_completed=NULL";
    // $result = $conn->query($sql);
    // $indx = 0;
    // if ($result->num_rows > 0)
    //     while($row = $result->fetch_assoc()) {
    //         $farr["requests_on"][$indx][2] = $farr["requests_on"][$indx][2].", "
    //                                         .$row["date_submitted"].", "
    //                                         .$row["category"].", "
    //                                         .$row["name"].", "
    //                                         .$row["num_people"];
    ...
    // Trucks
    $sql = "SELECT latitude , longitude FROM user WHERE role='RESCUER'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0)
        while($row = $result->fetch_assoc())
    ...
    if ($result->num_rows > 0)
        while($row = $result->fetch_assoc()) 
            $farr["lines"][] = [[floatval($row["la1"]), floatval($row["lo1"])],
                        [floatval($row["la2"]), floatval($row["lo2"])]
                        ,"blue"];

    $conn->close();
    //echo json_encode($farr);
    echo json_encode($farr, JSON_PRETTY_PRINT);
    //#echo "<pre>". json_encode($farr, JSON_PRETTY_PRINT) ."<pre/>";
    } else {
